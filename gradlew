...
if $darwin; then
    DEFAULT_JVM_OPTS='"-Xmx64m" "-Xms64m" "-Xdock:name=Gradle" "-Xdock:icon=$APP_HOME/media/gradle.icns"'
fi
...
if [ "$cygwin" = "true" -o "$msys" = "true" ] ; then
    ...
    # Add a / if CLASSPATH doesn't end with a slash.
    if echo "$CLASSPATH" | grep -q "/$" ; then
        CLASSPATH="$CLASSPATH"
    else
        CLASSPATH="$CLASSPATH/"
    fi
...
# For Darwin, add default JVM options
if $darwin; then
    DEFAULT_JVM_OPTS='"-Xmx64m" "-Xms64m" "-Xdock:name=Gradle" "-Xdock:icon=$APP_HOME/media/gradle.icns"'
fi
